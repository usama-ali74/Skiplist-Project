...
class Skiplist:  # making a class to implement Skiplist data structure
    def __init__(self):
        ...
        self.stack = [None for i in range(100)]  # self.stack is used to store the pointer for adding the new nodes in skiplist
    ...
    def find_pred_node(self, x):  # In func find_pred_node we find the given element in our skiplist
        ...
        while r >= 0:  # for searching the required node we run a while loop until we reach at base list
            while u.next[r] != None and u.next[r].value < x:  # In this line we also run a while loop with condition if the next node of skiplist should not be equal to None and value is is greater then the value of the skiplist
    ...
    def Add(self, x):  # Add func will add a node in the skiplist
        ...
        while r >= 0:  # for searching the required node we run a while loop until we reach at base list
            while u.next[r] != None and u.next[r].value < x:  # In this line we also run a while loop with condition if the next node of skiplist should not be equal to None and value is is greater then the value of the skiplist
                u = u.next[r]  # go right in list r
            if u.next[r] != None and u.next[r].value == x:  # In this condition we check if the new node value is already in the list
    ...
    def remove(self, x):  # Remove func will remove the node in skiplist
        ...
        while self.sentinel.height == 0 and len(self.sentinel.next) != 1:  # if lenght of sentinel is greater then 1 and height of sentinel is equal to 0 then
